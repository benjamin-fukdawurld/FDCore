...
class HashArray
{
	...
	public:
		...
		void insert(const key_type &key, const reference value)
		{
			...
			m_data.insert(std::lower_bound(m_data.begin(), m_data.end(), p,
		...
		{
			return find() != m_data.end();
		}


		const_iterator find(const key_type &key) const
		{
			const_iterator it = std::lower_bound(m_data.begin(), m_data.end(), p,
				[](const std::pair<size_t, value_type> &a, const std::pair<size_t, value_type> &b)
				{
					return a.first < b.first;
				});
		...
		{
			iterator it = std::lower_bound(m_data.begin(), m_data.end(), p,
				[](const std::pair<size_t, value_type> &a, const std::pair<size_t, value_type> &b)
			{
				return a.first < b.first;
			});
