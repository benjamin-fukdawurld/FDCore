...
namespace FDCore
{
	...
	class MemoryPool
	{
		...
		public:
			...
			Handle<T> allocate(Args ...args) { return allocateFrom<T, Args...>(0, args...); }
		...
		private:
			...
			{
				auto it = m_blocks.emplace(std::lower_bound(m_blocks.begin(), m_blocks.end(),
					[&position](const MemoryBlock &b) { return position < b.getPtr(); }));
				--it;
			...
			{
				obj->~T();
				FDCore::remove_if(m_blocks.begin(), m_blocks.end(), [obj](const MemoryBlock &b) { return b.isInBlock(obj); });
			}
