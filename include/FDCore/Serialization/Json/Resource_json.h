...
rapidjson::Value> FDJson::serialize(const T &res, Serializer &serializer)
{
    rapidjson::Value result(serializer.serialize(static_cast<const FDCore::Object &>(res)));
    std::string_view str = res.getResourceName();
    result.AddMember(serializer.serialize("resourceName"), serializer.serialize(str.data(), str.size()), serializer.getAllocator());
    std::string_view path = res.getResourcePath();
    if(!path.empty())
        result.AddMember(serializer.serialize("resourcePath"), serializer.serialize(path.data(), path.size()), serializer.getAllocator());
...
template<typename T>
std::enable_if_t<std::is_same_v<T, FDCore::AbstractResource> || std::is_same_v<T, FDCore::BaseResource>,
bool> FDJson::unserialize(const rapidjson::Value &val, T &res, Serializer &serializer, std::string *err)
{
    ...
    std::string path;
    if(!val.HasMember("resourcePath")
       || !serializer.unserialize(val["resourcePath"], path, err)
       || path.empty())
        return true;
