...
/**
*
* @file FDCore.h This file defines several basic tools like string modifications, regex for numbers, number of cores etc.
*
*/

/**
* @namespace FDCore contains all the utils functions and objects used in most of the FD framework's libraries
*/
namespace FDCore
{
	...
	*/
	inline std::string &prepareString(std::string &s)
	{
		return uniformSpaces(trim(s));
	}
	...
	*/
	...
	/**
	* @brief getNumberOfCores this function returns the number of cores for this application if NUMBER_OF_CORE is defined,
	* it is returned otherwise it returns std::thread::hardware_concurrency(). If NO_THREAD_ALLOWED is defined it returns 0
	...
	*/
	constexpr uint32_t fnv32_prime()
	{
		return static_cast<uint32_t>(FD_CORE_FNV32_PRIME);
	}
	...
	*/
	constexpr uint32_t fnv32_init()
	{
		return static_cast<uint32_t>(FD_CORE_FNV32_INIT);
	}
	...
	*/
	...
	{
		return reinterpret_cast<T*>((reinterpret_cast<unsigned int>(toAlign) + Align) & ~(Align - 1));
	}
