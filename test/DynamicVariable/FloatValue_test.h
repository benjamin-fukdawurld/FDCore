#ifndef FDCORE_FLOATVALUE_TEST_H
#define FDCORE_FLOATVALUE_TEST_H

#include <FDCore/DynamicVariable/FloatValue.h>
#include <gtest/gtest.h>
#include <iostream>

static constexpr FDCore::FloatValue::FloatType TEST_FLOAT_VALUE = 3.1415;

TEST(FloatValue_test, test_constructors)
{
    ASSERT_FLOAT_EQ(static_cast<float>(FDCore::FloatValue()), 0.0f);
    ASSERT_FLOAT_EQ(static_cast<float>(FDCore::FloatValue(TEST_FLOAT_VALUE)), TEST_FLOAT_VALUE);
    ASSERT_FLOAT_EQ(static_cast<float>(FDCore::FloatValue(2.0F)), 2.0F);

    FDCore::FloatValue value;
...
{
    ASSERT_TRUE(FDCore::FloatValue() == 0);
    ASSERT_TRUE(FDCore::FloatValue(TEST_FLOAT_VALUE) == TEST_FLOAT_VALUE);
    ASSERT_TRUE(FDCore::FloatValue(TEST_FLOAT_VALUE) <= TEST_FLOAT_VALUE);
    ASSERT_TRUE(FDCore::FloatValue(TEST_FLOAT_VALUE) < 2.0F * TEST_FLOAT_VALUE);
    ASSERT_TRUE(FDCore::FloatValue(TEST_FLOAT_VALUE) >= TEST_FLOAT_VALUE);
    ASSERT_TRUE(FDCore::FloatValue(TEST_FLOAT_VALUE) > 0);
}

TEST(FloatValue_test, test_plus_operators)
{
    float test = 1;
    FDCore::FloatValue value(1.0F);
...
{
    float test = 1;
    FDCore::FloatValue value(1);
    ASSERT_FLOAT_EQ(static_cast<float>(FDCore::FloatValue(value) *= 0), 0);
    ASSERT_FLOAT_EQ(static_cast<float>(value * 0), 0);
    ASSERT_FLOAT_EQ(static_cast<float>(FDCore::FloatValue(value) *= 1), static_cast<float>(value));
    ASSERT_FLOAT_EQ(static_cast<float>(value * 1), static_cast<float>(value));
    ASSERT_FLOAT_EQ(static_cast<float>(FDCore::FloatValue(value) *= -1),
...
TEST(FloatValue_test, test_division_operators)
{
    ...
    FDCore::FloatValue value(TEST_FLOAT_VALUE);
    ASSERT_FLOAT_EQ(static_cast<float>(FDCore::FloatValue(value) /= 1), static_cast<float>(value));
    ASSERT_FLOAT_EQ(static_cast<float>(value / 1), static_cast<float>(value));
    ASSERT_FLOAT_EQ(static_cast<float>(FDCore::FloatValue(value) /= -1), static_cast<float>(-value));
    ASSERT_FLOAT_EQ(static_cast<float>(value /= 2), test /= 2);
...
#endif // FDCORE_FLOATVALUE_TEST_H
