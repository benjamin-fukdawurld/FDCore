...
class FDCore_FileUtils_readFile : public ::testing::Test
{
  ...
  protected:
    ...
    static constexpr const char *getTextFilePath() { return "text_file.txt"; }

    static constexpr const char *getEmptyFilePath() { return "empty_file.txt"; }

    static constexpr const char *getNoFilePath() { return "no_file"; }
...
class FDCore_FileUtils_readBinaryFile : public ::testing::Test
{
  ...
  protected:
    struct TestStruct
    {
        ...
        TestStruct(int i, float f, double d, bool b, char c) : i(i), f(f), d(d), b(b), c(c) {}

        bool operator==(const TestStruct &t) const
        {
            return i == t.i && std::abs(f - t.f) <= std::numeric_limits<float>::epsilon() &&
                   std::abs(d - t.d) <= std::numeric_limits<double>::epsilon() && b == t.b &&
                   c == t.c;
        }
    };

  protected:
    static TestStruct getTestStruct() { return { 42, 3.14f, 1.23456, true, 'a' }; }

    static constexpr const char *getBinFilePath() { return "bin_file"; }

    static constexpr const char *getEmptyFilePath() { return "empty_bin_file"; }
...
TEST_F(FDCore_FileUtils_readFile, readFile_EmptyFileTest)
{
    std::unique_ptr<char[]> result =
      FDCore::readFile(FDCore_FileUtils_readFile::getEmptyFilePath());
